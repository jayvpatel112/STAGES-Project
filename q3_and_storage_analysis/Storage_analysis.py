...
def _(Path, pd, requests):
    ...
    def load_smard_hourly(start, end, cache_path="data/cache/smard_cache.pkl"):
    ...
    _raw_df = load_smard_hourly(start="2024-12-31", end="2026-01-02",
                                cache_path="data/cache/smard_cache_2025_wide.pkl")
...
def _(
    ...
    _ned = (storage_df[["net_export_gw"]]
            ...
            .reset_index(drop=True))
    _ned["ranked_hour"] = np.arange(1, len(_ned) + 1)
    _ned.to_csv("outputs/storage/storage_net_export_duration.csv", index=False)
    ...
    _mb.to_csv("outputs/storage/storage_monthly_balance.csv", index=False)
    ...
    _daily_stats.to_csv("outputs/storage/storage_daily_surplus_duration.csv", index=False)
    ...
    _dc.to_csv("outputs/storage/storage_duration_cost.csv", index=False)

    # --- 5. Import displacement curve ---
    displacement_df.to_csv("outputs/storage/storage_import_displacement.csv", index=False)

    # --- 6. Worst week: import gap and battery state of charge ---
    _wk_import = worst_event_df[["timestamp", "imports_gw"]].copy()
    _wk_import.to_csv("outputs/storage/storage_worst_week_import.csv", index=False)
    ...
    _wk_soc_wide.to_csv("outputs/storage/storage_worst_week_soc.csv", index=False)

    # --- 7. Arbitrage cumulative revenue ---
    arbitrage_df[["date", "cumulative_eur"]].to_csv("outputs/storage/storage_arbitrage_cumulative.csv", index=False)
...
def _(
    ...
    _events_export.to_csv("outputs/storage/storage_deficit_events.csv", index=False)
    ...
    _m["mwh_per_added_gwh"] = _m["extra_disp_twh"] * 1_000_000 / _m["added_cap_gwh"]
    _m = _m.dropna()
    _m[["capacity_gwh", "mwh_per_added_gwh"]].to_csv("outputs/storage/storage_marginal_displacement.csv", index=False)

    # --- 11. Imports remaining after storage ---
    _r = displacement_df.copy()
    _r["imports_remaining_twh"] = total_imports_twh - _r["imports_displaced_twh"]
    _r[["capacity_gwh", "imports_remaining_twh"]].to_csv("outputs/storage/storage_imports_remaining.csv", index=False)
    ...
    battery_utilization_df[["capacity_gwh", "full_cycles_per_year", "avg_soc_pct"]].to_csv(
        "outputs/storage/storage_battery_utilization.csv", index=False
    )
    ...
    _ms.to_csv("outputs/storage/storage_monthly_arbitrage.csv", index=False)
    ...
    _sens_wide.to_csv("outputs/storage/storage_payback_sensitivity.csv", index=False)
    ...
    _soc_wide.to_csv("outputs/storage/storage_soc_full_year.csv", index=False)
