...
def _(pd):
    df = pd.read_csv(
        "Actual_generation_202507010000_202510010000_Hour.csv",
        sep=";",
        thousands=",",
        encoding="utf-8-sig"
    )
...
def _(df_clean):
    ...
    monthly_mix.to_csv("q3_monthly_mix.csv", index=False)
...
def _(df_clean, pd):
    df_demand = pd.read_csv(
        "Actual_consumption_202507010000_202510010000_Hour.csv",
...
@app.cell
def _(pd):
    weather_hamburg = pd.read_csv("weather_hamburg.csv", skiprows=2)
    weather_hamburg
    return
...
def _(pd):
    cities = {
        "Hamburg":   "weather_hamburg.csv",
        "Munich":    "weather_munich.csv",
        "Berlin":    "weather_berlin.csv",
        "Frankfurt": "weather_frankfurt.csv"
    }
...
def _(df_merged, model_solar, model_wind, np, pd):
    ...
    scatter_export.to_csv("q3_weather_scatter.csv", index=False)
    ...
    wind_curve_export.to_csv("q3_wind_curve.csv", index=False)
    ...
    solar_curve_export.to_csv("q3_solar_curve.csv", index=False)
...
@app.cell
def _(df_merged):
    df_merged.to_csv("merged_q3_2025.csv", index=False)
